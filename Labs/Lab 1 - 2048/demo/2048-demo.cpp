...
public:
	 ...
	 */
	void make_statistic(size_t n, const board& b, int score, int unit, std::ofstream& file) {
...
int main(int argc, const char* argv[]) {
	...
	std::string loadname = "weights-demo.bin";
	tdl.load(loadname);
	std::cout << "Load the model at " << loadname << std::endl;
	
	// open csv file
	std::string filename = "scores-demo.csv";
	std::ofstream file;
	file.open(filename);
	std::cout << "Score has been written to " << filename << std::endl;
	file << "episode" << "," << "mean score" << std::endl;
	...
	for (size_t n = 1; n <= total; n++) {
		...
		tdl.make_statistic(n, b, score, 100, file);
		path.clear();
	}

	// store the model into file
//	tdl.save("weights-demo.bin");
