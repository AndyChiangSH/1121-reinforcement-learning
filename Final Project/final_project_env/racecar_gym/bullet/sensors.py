...
class Lidar(BulletSensor[NDArray[(Any,), np.float32]]):
    @dataclass
    class Config:
        accuracy: float
        rays: int
    ...
    def observe(self) -> NDArray[(Any,), np.float32]:
...
class RGBCamera(BulletSensor[NDArray[(Any, Any, 3), np.int32]]):
    ...
    def observe(self) -> NDArray[(Any, Any, 3), np.int32]:
        ...
        return rgb_array.astype(dtype=np.uint8)


class AccelerationSensor(BulletSensor[NDArray[(6,), np.float32]]):
    @dataclass
    class Config:
        time_delta: float
    ...
    def observe(self) -> NDArray[(6,), np.float32]:
...
class VelocitySensor(BulletSensor[NDArray[(6,), np.float32]]):
    @dataclass
    class Config:
        gaussian_noise: float
...
class PoseSensor(BulletSensor[NDArray[(6,), np.float32]]):
    @dataclass
    class Config:
