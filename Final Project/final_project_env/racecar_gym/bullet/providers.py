...
from racecar_gym.bullet.actuators import BulletActuator, Motor, SteeringWheel, Speed, MotorCompetition, \
    SteeringWheelCompetition
...
def load_actuator(config: ActuatorConfig) -> BulletActuator:
    ...
    if config.type == 'motor_competition':
        return MotorCompetition(name=config.name, config=Motor.Config(**config.params))
    if config.type == 'steering_competition':
        return SteeringWheelCompetition(name=config.name, config=SteeringWheel.Config(**config.params))
